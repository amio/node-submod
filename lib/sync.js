...
function classifyArgs (vars) {
  'use strict' // allow let in Node.js 4
  ...
  vars.args.forEach(function (arg) {
    ...
    if (filesPattern.test(arg)) {
      ...
      deltaMilliseconds = parseInt(parseFloat(arg.substr(1)) * 1000, 10)
      delta.push(deltaMilliseconds)
    } else if (deltaComplex.test(arg)) {
      deltaMilliseconds = parseComplexDelta(arg.substr(1))
      delta.push(deltaMilliseconds)
    }
  })
...
function syncSubtitles (vars) {
  //
  var jobs = vars.files.map(function (filename) {
...
function shiftSubtitle (meta, opts) {
  ...
  console.log('DONE: %s\n  --> %s', meta.filename, chalk.yellow(submodFilename))
}
...
function parseComplexDelta (deltaString) {
  ...
  const ms = parts.reduce(function (prev, curr) {
    switch (curr.substr(-1)) {
      case 'h':
        return prev + parseInt(curr.slice(0, -1), 10) * 60 * 60 * 1000
      case 'm':
        return prev + parseInt(curr.slice(0, -1), 10) * 60 * 1000
      case 's':
        return prev + parseInt(curr.slice(0, -1), 10) * 1000
      case 'l':
        return prev + parseInt(shifter.charPadding(curr.slice(0, -1), 'right', '0', 3), 10)
    }
  }, 0)

  return parseInt(deltaString.substr(0, 1) + ms, 10)
}
